...
It's very important for data science projects and code to be sustainable. If your project is only 60% rigor, there has to be a way to extend the project to 90% rigor. At the end of telling your story and pitching your analysis, you can talk about follow-ups. Ideally, there will be a framework so capable non-experts can extend the analysis. Here, web apps help tremendously because it sorts out what's important. If your studying Twitter sentiment trends on the [#BARTstrike](https://twitter.com/search?q=%23BARTstrike&src=hash), the first go around of your data science pipeline may have deemed geographic analysis as a rabbit hole. If the client is interested in that type of analysis, you can go through the process again. Eventually, you'll reach 90% rigor while matching the business cycle. 

## <a id="week7"></a>Week 7: 

### Techniques learned: networkx, Pig, Hive, Amazon EMR

### Topics covered: graphs, YARN

This week we implemented graph search using different queues on regions in San Francisco and a citation network of physicists ([the Erdos number](http://www.oakland.edu/enp/)). We then leveraged networkx and again received shock at how slow our own implementations were. We did more big data stuff in Pig and Hive, they seem to be Python-ish and SQL-ish languages with greater latency. 

A significant chunk of time were spent getting ready for our personal week long project for next week. We wrote proposals for our projects to simulate a situation in a data science job where we have to convince the manager to fund project so and so. Many companies are still hesitant to invest 150K+ / year on a data scientist with uncertain ROI. Contrast that to a software engineer or a UX researcher where you know you will likely get steady production. So many successful data scientists emphasize the "black art" needed not only for tuning your models but solving and visualizing problems that have immediate impact. On good days, a data scientist will completely reframe what it can do with the deluge of data a company has a hold on, while on bad days a data scientist might be wasting everyone's time working on an interesting project. 
